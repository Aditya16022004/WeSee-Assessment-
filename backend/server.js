...
const IS_LOCAL_HARDHAT = (process.env.CHAIN_ID === '1337') || /127\.0\.0\.1|localhost/.test(process.env.RPC_URL || '');
...
const normalizedPrivateKey = (process.env.BACKEND_PRIVATE_KEY || '').startsWith('0x')
...
app.post('/add-dummy-usdt', async (req, res) => {
    try {
        ...
        // Use backend/deployer account to give USDT to user
...
app.post('/give-backend-usdt', async (req, res) => {
    try {
        ...
        // Use backend/deployer account to give USDT to backend
        const deployerWallet = wallet;
...
app.post('/match/stake', async (req, res) => {
	try {
		if (!IS_LOCAL_HARDHAT) {
			return res.status(400).json({ error: 'This endpoint is only available on local Hardhat network.' });
		}
